...
func get_hand_debug_snapshot() -> Dictionary:
	...
	for side in _HAND_SIDES:
		...
		hand_snapshot[side] = {
			...
			"stale_ms": int(payload.get("stale_ms", 0)),
			"grace_frames": int(payload.get("grace_frames", 0)),
			"grace_ms": int(payload.get("grace_ms", 0)),
			"stable_ms": int(payload.get("stable_ms", 0)),
			"predicted": bool(payload.get("predicted", false)),
			"fresh_sample": bool(payload.get("fresh_sample", false)),
			"sample_source": str(payload.get("sample_source", "none")),
			"source_frame_index": int(payload.get("source_frame_index", -1)),
